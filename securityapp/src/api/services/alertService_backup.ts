...
export const alertService = {
  ...
  // Get recent alerts only (for refreshing alerts list)
  getRecentAlerts: async (limit: number = 5): Promise<Alert[]> => {
    try {
      ...
      // Sort by created_at (most recent first) and take the limit
      ...
      console.log(`✅ Fetched ${recentAlerts.length} recent alerts from ${alertsData.length} total alerts`);
  ...
  resolveAlert: async (id: string | number): Promise<Alert> => {
    try {
      ...
      const resolvedAlert = {
        ...
        status: 'completed', // Force status to completed for resolved alerts
